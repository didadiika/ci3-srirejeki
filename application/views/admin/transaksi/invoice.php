...
$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
  ...
  var table_ketan = $('#tabel-ketan').DataTable({
     "language": {
        ...
        "order": [],
        "ajax": {
              url:"<?php echo base_url('invoice/tampil');?>",
              type:"GET",
        },
        "columnDefs": [ {
            "targets": 0,
  ...
  var table_ir = $('#tabel-ir').DataTable({
     "language": {
        ...
        "order": [],
        "ajax": {
              url:"<?php echo base_url('invoice/tampil_ir');?>",
              type:"GET",
        },
        "columnDefs": [ {
            "targets": 0,
...
$('#tabel-ir').on('click','.item_selesai',function(){
        ...
        $('#btn_hapus').on('click',function(){
          $('#btn_hapus').prop("disabled",true);
            ...
            $.ajax({
            type : "POST",
            url  : "<?php echo base_url('invoice/hapus/')?>"+id,
            data : {id_hapus: id},
                    success: function(data){
...
<button type="button" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true">&times;</span></button>
<h4 class="modal-title">Form Input Invoice</h4>
</div>
<form action="<?php echo base_url('invoice/buat') ;?>" method="post" id="form-buat" autocomplete="off">
<div class="modal-body">

      <div id="tagNama" class="form-group">
          <label class="control-label" for="inputError"><i id="iconNama"></i> Tanggal Invoice</label>
          <input type="text" name="tanggal" id="tgl" class="form-control" placeholder="Tanggal" required
          data-date-format="dd-mm-yyyy" value="<?php echo date("d-m-Y");?>" data-date-end-date="0d">
          <span class="help-block" id="pesanNama"></span>
      </div>
      ...
      <div id="tagNama" class="form-group">
          ...
          <select name="id_pelanggan"  class="form-control select2" style="width: 100%;" id="id_pelanggan" required>
          </select>
          <span class="help-block" id="pesanNama"></span>
      </div>
...
<div class="alert alert-success alert-dismissible">
      ...
      <h4><i class="icon fa fa-info"></i> Catatan</h4>
               Hanya Invoice yang ber status <strong>Selesai</strong> yang bisa dicetak. 
</div>
